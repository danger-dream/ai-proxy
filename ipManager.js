...
class IPManager {
    constructor() {
        this.BLACK_LIST_FILE = path.join(__dirname, 'black.json');
    ...
    recordIPError(ip, errorType) {
        ...
        if (this.ipData.ip_error_counter[ip].count >= this.IP_ERROR_THRESHOLD && 
            now - this.ipData.ip_error_counter[ip].firstError <= this.ERROR_WINDOW) {
